...
const UNITS = [
  'index.html', '02-mindset.html', '03-goals.html', '04-ai3p.html',
...
for (const file of UNITS) {
  ...
  unitIndex.set(file, sections.length);       // 目錄排在最後，單元索引就是目前長度
  sections.push(...found);
  console.log(`  ${file.padEnd(20)} ${String(found.length).padStart(2)} 張`);
}

/* 第 1 張是封面，目錄放在最後一張（按 Space 才跳過去）。單檔版不能跳別的 .html
   （dist 底下沒有那些檔，而且換頁會讓全螢幕被瀏覽器結束），
   所以把目錄的 <a href> 改成同頁跳轉的 data-goto。 */
const tocSection = (readFileSync(join(ROOT, 'toc.html'), 'utf8').match(RE_SECTION) || [])[0];
if (!tocSection) throw new Error('toc.html 找不到目錄 <section class="slide">');
...
sections.push(toc);
console.log(`  ${'toc.html（目錄）'.padEnd(18)}  1 張`);
